...
def run_pipeline(symbol="MSFT"):
    ...
    daily_data = data["Time Series (Daily)"]           #daily_data: it is now a dictionary with date as key and OHLC and volume as values  
    # print(daily_data)
    ...
    df = pd.DataFrame.from_dict(daily_data, orient="index")       #orient=index: this makes keys as rows/index and values as columns in the dataframe
    df.columns = ['open', 'high', 'low', 'close', 'volume']        #renaming column names
    df.index = pd.to_datetime(df.index)                           #converts rows/index from string to proper datetime format for sorting, filtering etc
    df.head()

    df = df.sort_index()         #now the tail has the latest i.e today's data
    lastSevenDays = df.tail(7)    #lastSevenDays: the last seven days from the 100 data in df
    lastSevenDays.head()

    lastSevenDays = lastSevenDays.ffill()   #ffill(): Forward fill, fills a NaN value with previous rows values
    ...
    lastSevenDays['daily_return'] = ((lastSevenDays['close'] - lastSevenDays['open']) / lastSevenDays['open']) * 100
